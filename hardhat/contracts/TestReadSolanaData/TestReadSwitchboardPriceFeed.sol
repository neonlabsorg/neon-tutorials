...
contract TestReadSwitchboardPriceFeed {
    using SolanaDataConverterLib for bytes;
    using SolanaDataConverterLib for uint64;
    using SolanaDataConverterLib for uint128;
    ...
    function readSolanaPythPriceFeed(bytes32 solanaAddress, uint64 offset, uint64 len) public view returns(int128, int64) {
        (bool success, bytes memory data) = QueryAccount.data(uint256(solanaAddress), offset, len);
        require(success, "failed to query account data");

        return (
            (data.toUint128(366)).readLittleEndianSigned128(),
            (data.toUint64(358)).readLittleEndianSigned64()
        );
    }
}