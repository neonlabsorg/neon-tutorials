...
async function main() {
    let SOLANA_NODE;
    let TestICSFlowWithOrcaSwapAddress;
    ...
    if (network.name == "neonmainnet") {
        ...
        TestICSFlowWithOrcaSwapAddress = config.CALL_SOLANA_SAMPLE_CONTRACT_MAINNET;
        ...
        TestICSFlowWithOrcaSwapAddress = config.CALL_SOLANA_SAMPLE_CONTRACT;
    ...
    const [address] = web3.PublicKey.findProgramAddressSync(
        [
            new web3.PublicKey('1111111111112Lkye6dEUPqgnf19WqypFfa6b5r8').toBuffer(), 
    ...
    const TestICSFlowWithOrcaSwapFactory = await ethers.getContractFactory("TestICSFlowWithOrcaSwap");
    let TestICSFlowWithOrcaSwap;
    let solanaTx;
    let tx;
    let receipt;

    if (ethers.isAddress(TestICSFlowWithOrcaSwapAddress)) {
        TestICSFlowWithOrcaSwap = TestICSFlowWithOrcaSwapFactory.attach(TestICSFlowWithOrcaSwapAddress);
    } else {
        TestICSFlowWithOrcaSwap = await ethers.deployContract("TestICSFlowWithOrcaSwap");
        await TestICSFlowWithOrcaSwap.waitForDeployment();

        TestICSFlowWithOrcaSwapAddress = TestICSFlowWithOrcaSwap.target;
        console.log(
            `TestICSFlowWithOrcaSwap deployed to ${TestICSFlowWithOrcaSwap.target}`
        );
    }

    const SenderAccount = await web3.PublicKey.createWithSeed(
        new web3.PublicKey(contractPublicKey), 
        await TestICSFlowWithOrcaSwap(), 
        TOKEN_PROGRAM_ID
    );
    console.log(SenderAccount, 'SenderAccount');

    const contractPublicKeyInBytes = await TestICSFlowWithOrcaSwap.getNeonAddress(TestICSFlowWithOrcaSwapAddress);
    ...
    tx = await TestICSFlowWithOrcaSwap.connect(user1).execute(
